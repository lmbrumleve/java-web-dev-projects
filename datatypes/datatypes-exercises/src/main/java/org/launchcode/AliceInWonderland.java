package org.launchcode;
import java.util.Scanner;

public class AliceInWonderland {

    public static void main(String[] args) {
        String aliceInWonderland = "Alice was beginning to get very tired of sitting " +
                "by her sister on the bank, and of having nothing to do: " +
                "once or twice she had peeped into the book her sister was reading, " +
                "but it had no pictures or conversations in it, ‘and what is the use " +
                "of a book,’ thought Alice ‘without pictures or conversation?’";

        Scanner input = new Scanner(System.in);
        System.out.println("Which word would you like to search for? ");
        String searchWord = input.nextLine();
        String lowerCaseSearchWord = searchWord.toLowerCase();
        String lowerCaseAlice = aliceInWonderland.toLowerCase();
        Boolean foundIt = lowerCaseAlice.contains(lowerCaseSearchWord);
        System.out.println(foundIt);

        Integer index = lowerCaseAlice.indexOf(lowerCaseSearchWord);
        System.out.println(index);
        String aliceMinusSearchWord = aliceInWonderland.replace(searchWord, "");
        System.out.println(aliceMinusSearchWord);
    }
}